...
import { gfx, Quat, Vec2, Vec3 } from "cc";
import { GSplatResourceBase } from './gspat-resource-base';
...
export class GSplatResource extends GSplatResourceBase {
    ...
    constructor(gsplatData) {
        ...
        this.colorTexture = this.createTexture('splatColor', gfx.Format.RGBA16F, size);
        this.transformATexture = this.createTexture('transformA', gfx.Format.RGBA32UI, size);
        this.transformBTexture = this.createTexture('transformB', gfx.Format.RGBA16F, size);
    ...
    updateColorData(gsplatData) {
        ...
        const data = new Uint16Array(texture.width * texture.height * 4);
    ...
    updateTransformData(gsplatData) {
        ...
        const dataA = new Uint32Array(this.transformATexture.width * this.transformATexture.height * 4);
        const dataAFloat32 = new Float32Array(dataA.buffer);
        const dataB = new Uint16Array(this.transformBTexture.width * this.transformATexture.height * 4);
        ...
        this.uploadTextureData(this.transformATexture, dataA)
        this.uploadTextureData(this.transformBTexture, dataB)
    }
