...
import { GSplatData } from "./scene/gsplat-data";
...
const readFloatPly = async (buffer: ArrayBuffer, readState, elements, comments) => {
    ...
    buffer = buffer.slice(readState.byteOffset)
    let byteLength = buffer.byteLength

    const toRead = element.count;
    const toReadByteLength = toRead * inputSize;

    if (byteLength < toReadByteLength) {
        console.warn('data is not enough')
        debugger
    }

    let floatData = new Float32Array(buffer, 0, element.count * numProperties);


    for (let j = 0; j < numProperties; ++j) {
        const s = storage[j];
        for (let n = 0; n < toRead; ++n) {
            s[n] = floatData[n * numProperties + j];
        }
    }

    readState.byteOffset += toReadByteLength
...
export class PlyParser {
    async parse(asset: BufferAsset, propertyFilter = null) {
        let buffer = asset.buffer()
        let uint8Data = new Uint8Array(buffer)

        if (!startsWith(uint8Data, magicBytes)) {
            throw new Error('Invalid ply header');
        }

        // search for end-of-header marker
        let headerLength = find(uint8Data, endHeaderBytes);
        if (headerLength === -1) {
            throw new Error('Invalid ply header');
        }
        ...
        const lines = new TextDecoder('ascii')
            .decode(uint8Data.subarray(0, headerLength))
            .split('\n');
        ...
        readState.byteOffset = headerLength + endHeaderBytes.length;

        if (!propertyFilter) {
            propertyFilter = defaultElementFilter
        }

        const readData = async (): Promise<GSplatData> => {
            // load compressed PLY with fast path
            if (isCompressedPly(elements)) {
                return await readCompressedPly(uint8Data, elements, comments);
            }
            ...
            if (isFloatPly(elements)) {
                return await readFloatPly(buffer, readState, elements, comments);
            }

            // fallback, general case
            return await readGeneralPly(buffer, elements, comments);
        };
...
export const plyParser = new PlyParser
