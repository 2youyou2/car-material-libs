import { Color, gfx, utils, Vec3 } from "cc";
...
export class PCDLoader {
    ...
    parse(data: ArrayBuffer) {
        ...
        let mesh = utils.createMesh({
            ...
            positions: position,
            normals: normal.length > 0 ? normal : undefined,
            colors: color.length > 0 ? color : undefined,

            customAttributes: [{
                attr: new gfx.Attribute('a_intensity', gfx.Format.R32F),
                values: intensity
            }],
...
export const pcdLoader = new PCDLoader()